    ...
    def stream_json_file(self, json_file_name, output_file_name="parsed_tweets.csv", stop_at=1000000000, verbose=False):
        ...
        for i in open(json_file_name):
            ...
            try:
                ...
                if current_tweet['coordinates'] != None and current_tweet['coordinates']['coordinates'] != None and len(
                ...
                ######################################################################################################
                # Assemble the row
                cleaned_current_tweet = [id, text, created_at, language, retweet_count, screen_name, country,
                                         followers_count, time_zone, account_location, longitude, lattitude, name]
